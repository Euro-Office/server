...
'use strict';
if (require('../../Common/sources/runtime/profile').isMemoryRuntime()) {
  ...
  /**
   * Set status to `status` and clear the callback column to empty string.
   * Used by the shutdown-recovery path (changes2forgotten.js) where the doc
   * must be force-saved without re-firing the original application callback.
   * Unlike `update()`, this performs a direct SET on the callback column rather
   * than an append through the UserCallback wire format.
   * @param {object} ctx
   * @param {string} docId
   * @param {number} status
  ...
  exports.updateStatusAndClearCallback = updateStatusAndClearCallback;
